...
export class DevicesService {
  ...
  async create(
    ...
    this.logger.debug(`Create device payload: ${JSON.stringify(createDeviceDto)}`);

    // Validate required fields
    if (!createDeviceDto.macAddress) {
      this.logger.error(`Device creation failed: Missing macAddress for user ${userId}`);
      throw new BadRequestException('MAC address is required');
    }
    if (!createDeviceDto.name) {
      this.logger.error(`Device creation failed: Missing name for user ${userId}`);
      throw new BadRequestException('Device name is required');
    }
    if (createDeviceDto.x === undefined || createDeviceDto.x === null) {
      this.logger.error(`Device creation failed: Missing x coordinate for user ${userId}`);
      throw new BadRequestException('X coordinate is required');
    }
    if (createDeviceDto.y === undefined || createDeviceDto.y === null) {
      this.logger.error(`Device creation failed: Missing y coordinate for user ${userId}`);
    ...
    try {
      ...
      if (existingDevice) {
        this.logger.warn(`Device creation failed: MAC address ${createDeviceDto.macAddress} already exists (owned by user ${existingDevice.userId})`);
      ...
      this.logger.log(`Device created successfully: ${device.id} (${device.name}) for user ${userId}`);
      return device;
    } catch (error) {
      this.logger.error(`Failed to create device for user ${userId}: ${error.message}`);
      throw error;
    }
  }
  ...
  async findDevicesByZone(
    ...
    if (!zoneId) {
      this.logger.error(`Zone devices lookup failed: Missing zone ID for user ${userId}`);
    ...
    try {
      ...
      this.logger.log(`Found ${devices.length} devices in zone ${zoneId} for user ${userId}`);
      return devices;
    } catch (error) {
      this.logger.error(`Failed to fetch devices for zone ${zoneId} and user ${userId}: ${error.message}`);
      throw error;
    }
  }
  ...
  async findDevicesByUser(userId: string): Promise<PrismaDevice[]> {
    ...
    try {
      ...
      this.logger.error(`Failed to fetch devices for user ${userId}: ${error.message}`);
      throw error;
    }
  }
  ...
  async findByMacAndUser(
    ...
    if (!macAddress) {
      this.logger.error(`MAC device lookup failed: Missing MAC address for user ${userId}`);
      throw new BadRequestException('MAC address is required');
    }
    if (!userId) {
      this.logger.error(`MAC device lookup failed: Missing user ID for MAC ${macAddress}`);
      throw new BadRequestException('User ID is required');
    }
  ...
  async update(
    ...
    this.logger.debug(`Update device payload: ${JSON.stringify(updateDeviceDto)}`);

    if (!id) {
      this.logger.error(`Device update failed: Missing device ID for user ${userId}`);
      throw new BadRequestException('Device ID is required');
    }
    if (!userId) {
      this.logger.error(`Device update failed: Missing user ID for device ${id}`);
      throw new BadRequestException('User ID is required');
    }
    if (!updateDeviceDto || Object.keys(updateDeviceDto).length === 0) {
      this.logger.error(`Device update failed: Empty update payload for device ${id} by user ${userId}`);
    ...
    try {
      ...
      if (!device) {
        this.logger.warn(`Device update failed: Device ${id} not found for user ${userId}`);
      ...
      this.logger.log(`Updating device ${id} (${device.name}) for authorized user ${userId}`);
      ...
      this.logger.error(`Failed to update device ${id} for user ${userId}: ${error.message}`);
      throw error;
    }
  }
  ...
  async remove(id: string, userId: string): Promise<PrismaDevice> {
    ...
    if (!id) {
      this.logger.error(`Device deletion failed: Missing device ID for user ${userId}`);
      throw new BadRequestException('Device ID is required');
    }
    if (!userId) {
      this.logger.error(`Device deletion failed: Missing user ID for device ${id}`);
      throw new BadRequestException('User ID is required');
    }
    ...
    try {
      ...
      if (!device || device.userId !== userId) {
        this.logger.warn(`Device deletion failed: Device ${id} not found or access denied for user ${userId}`);
        throw new Error('Device not found or access denied');
      }

      this.logger.log(`Deleting device ${id} (${device.name}) for authorized user ${userId}`);
      ...
      this.logger.error(`Failed to delete device ${id} for user ${userId}: ${error.message}`);
      throw error;
    }
  }
}
